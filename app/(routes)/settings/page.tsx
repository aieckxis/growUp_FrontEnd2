...
const generateAlerts = (
  ...
  if (thresholds.airPressure && sensor.airPressure !== undefined) {
    if (sensor.airPressure < thresholds.airPressure.min) {
      alerts.push({ id: alertId++, type: "warning", severity: "low", title: "Air Pressure Low", message: `Air pressure is ${sensor.airPressure} hPa. Below optimal range.`, time: timeAgo(now) })
    }
    if (sensor.airPressure > thresholds.airPressure.max) {
      alerts.push({ id: alertId++, type: "warning", severity: "low", title: "Air Pressure High", message: `Air pressure is ${sensor.airPressure} hPa. Above optimal range.`, time: timeAgo(now) })
    }
  }
...
const checkSystemStatus = (alerts: AlertData[]): 'Optimal' | 'Alerts Active' => {
  if (alerts.length === 0) return 'Optimal';
...
const loadState = (): { 
  ...
  return { controls: INITIAL_CONTROLS_FULL, activePreset: "balanced", controlMode: 'AUTOMATIC', thresholds: INITIAL_THRESHOLDS };
};

const saveState = (state: { controls: SystemControls, activePreset: string, thresholds: ThresholdState, controlMode: ControlMode }) => {
  try {
...
const useAquaponicsSettings = () => {
  ...
  const setControls = (newControls: SystemControls) => { setState(prevState => ({ ...prevState, controls: newControls })); setHasChanges(true); };
  const setThresholds = (newThresholds: ThresholdState) => { setState(prevState => ({ ...prevState, thresholds: newThresholds })); setHasChanges(true); }
  const setControlMode = (newMode: ControlMode) => { setState(prevState => ({ ...prevState, controlMode: newMode })); setHasChanges(true); }
  const setPreset = (newPreset: string) => { setState(prevState => ({ ...prevState, activePreset: newPreset })); setHasChanges(true); }
  ...
  return { controls: state.controls, activePreset: state.activePreset, thresholds: state.thresholds, controlMode: state.controlMode, setControls, setThresholds, setControlMode, setPreset, handleSave, hasChanges, setHasChanges };
};
...
const ControlToggle: React.FC<ControlToggleProps> = ({ label, description, icon: Icon, active, onChange, loading = false, disabled = false }) => (
  <div className={`flex items-center justify-between p-4 rounded-xl ${disabled ? 'bg-gray-100 opacity-60' : 'bg-gray-50'}`}>
    ...
    <button
      ...
      className={`w-12 h-6 rounded-full transition-colors flex-shrink-0 ${disabled ? 'bg-gray-300 cursor-not-allowed' : active ? 'bg-emerald-500' : 'bg-gray-300'} ${loading ? 'opacity-60 cursor-not-allowed' : ''}`}
    >
...
const PresetCard: React.FC<PresetCardProps> = ({ title, description, icon: Icon, active, onActivate, disabled = false }) => (
  <button
    ...
    disabled={disabled}
    className={`relative w-full p-4 rounded-xl border-2 transition-all text-left ${disabled ? 'border-gray-200 bg-gray-50 opacity-50 cursor-not-allowed' : active ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-gray-200 bg-white hover:border-gray-300'}`}
  >
...
const ThresholdRangeInput: React.FC<ThresholdRangeInputProps> = ({ label, unit, icon: Icon, minValue, maxValue, minLimit, maxLimit, onMinChange, onMaxChange }) => (
  <div className="p-4 bg-gray-50 rounded-xl">
    ...
    <div className="flex items-center gap-3">
      <div className="flex-1">
        ...
        <input type="number" value={minValue} onChange={(e) => onMinChange(Number(e.target.value))} min={minLimit} max={maxLimit} step="0.1" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
      </div>
      <span className="text-gray-400 font-medium mt-5">—</span>
      <div className="flex-1">
        <span className="text-xs text-gray-500">Max</span>
        <input type="number" value={maxValue} onChange={(e) => onMaxChange(Number(e.target.value))} min={minLimit} max={maxLimit} step="0.1" className="w-full mt-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500" />
      </div>
    </div>
...
export default function SettingsPage() {
  const { controls, activePreset, thresholds, controlMode, setControls, setThresholds, setControlMode, setPreset, handleSave, hasChanges } = useAquaponicsSettings();
  ...
  const handleControlChange = async (key: keyof SystemControls, val: boolean) => {
    ...
    if (controlMode === 'AUTOMATIC') {
      ...
      try {
        ...
        if (!modeResponse.success) console.error('⚠️ Failed to set MANUAL mode on backend:', modeResponse.error)
      } catch (error) {
    ...
    try {
      ...
      if (!response.success) throw new Error(response.error || 'Failed to control device')
  ...
  const applyPresetControls = async (presetControls: SystemControls) => {
    ...
    if (errors.length > 0) setErrorMessage(`Failed to apply controls: ${errors.join(', ')}`)
  }
  ...
  const handleSaveChanges = async () => {
    ...
    if (success) { alert('Settings saved successfully and synced!'); }
  ...
  const handleThresholdChange = (key: keyof ThresholdState, type: 'min' | 'max', val: number) => {
    setThresholds({ ...thresholds, [key]: { ...thresholds[key], [type]: val } });
  }
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-900">
                  Control Mode: <span className={controlMode === 'AUTOMATIC' ? 'text-blue-600' : 'text-orange-600'}>{controlMode}</span>
                </h3>
              </div>
              <button
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (!response.success) throw new Error(response.error || 'Failed to change system mode')
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${modeChanging ? 'bg-gray-400 cursor-not-allowed text-white' : controlMode === 'AUTOMATIC' ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-orange-600 hover:bg-orange-700 text-white'}`}
              >
                {modeChanging ? 'Switching...' : `Switch to ${controlMode === 'AUTOMATIC' ? 'Manual' : 'Automatic'}`}
              </button>
            </div>
            <p className="text-xs text-gray-600">
              {controlMode === 'AUTOMATIC' ? `🤖 AUTOMATIC mode: Use presets to control the system. Manual controls are disabled.` : '✋ MANUAL mode: You have full control. Use toggles below to control devices individually.'}
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              {controlMode === 'MANUAL' && <span className="text-xs text-orange-600 font-semibold">⚠️ Disabled in Manual Mode</span>}
            </div>
            {presetWarning && (
              <div className="mb-3 p-3 bg-orange-50 border border-orange-200 rounded-lg flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 text-orange-600 flex-shrink-0 mt-0.5" />
                <div className="flex-1"><p className="text-xs text-orange-700">{presetWarning}</p></div>
                <button onClick={() => setPresetWarning(null)} className="text-orange-600 hover:text-orange-800"><X className="w-4 h-4" /></button>
              </div>
            )}
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              {controlMode === 'AUTOMATIC' && <span className="text-xs text-blue-600 font-semibold">⚠️ Disabled in Automatic Mode</span>}
            </div>
            {errorMessage && (
              <div className="mb-3 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
                ...
                <button onClick={() => setErrorMessage(null)} className="text-red-600 hover:text-red-800"><X className="w-4 h-4" /></button>
              </div>
            )}
            {controlMode === 'AUTOMATIC' && (
              <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-start gap-2">
                <AlertTriangle className="w-4 h-4 text-blue-600 flex-shrink-0 mt-0.5" />
                <div className="flex-1"><p className="text-xs text-blue-700">Manual controls are disabled. Use presets above or switch to MANUAL mode for individual control.</p></div>
              </div>
            )}
            <div className="space-y-3">
              <ControlToggle label="Dosing Pump" description="Main water circulation" icon={Waves} active={controls.pump} onChange={(val) => handleControlChange('pump', val)} loading={controlLoadingStates.pump} disabled={controlMode === 'AUTOMATIC'} />
              <ControlToggle label="DC Fan" description="Air circulation & cooling" icon={Wind} active={controls.fan} onChange={(val) => handleControlChange('fan', val)} loading={controlLoadingStates.fan} disabled={controlMode === 'AUTOMATIC'} />
              <ControlToggle label="Grow Light" description="LED lighting system" icon={Sun} active={controls.growLight} onChange={(val) => handleControlChange('growLight', val)} loading={controlLoadingStates.growLight} disabled={controlMode === 'AUTOMATIC'} />
              <ControlToggle label="Humidity Controller" description="Automatic humidity management" icon={Droplets} active={controls.humidityController} onChange={(val) => handleControlChange('humidityController', val)} loading={controlLoadingStates.humidityController} disabled={controlMode === 'AUTOMATIC'} />
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            ...
            <div className="space-y-3">
              <ThresholdRangeInput label="Water Temperature" unit="°C" icon={Thermometer} minValue={thresholds.waterTemp.min} maxValue={thresholds.waterTemp.max} minLimit={18} maxLimit={30} onMinChange={(val) => handleThresholdChange('waterTemp', 'min', val)} onMaxChange={(val) => handleThresholdChange('waterTemp', 'max', val)} />
              <ThresholdRangeInput label="Air Temperature" unit="°C" icon={Wind} minValue={thresholds.airTemp.min} maxValue={thresholds.airTemp.max} minLimit={15} maxLimit={35} onMinChange={(val) => handleThresholdChange('airTemp', 'min', val)} onMaxChange={(val) => handleThresholdChange('airTemp', 'max', val)} />
              <ThresholdRangeInput label="pH Level" unit="" icon={Droplets} minValue={thresholds.ph.min} maxValue={thresholds.ph.max} minLimit={6.0} maxLimit={8.0} onMinChange={(val) => handleThresholdChange('ph', 'min', val)} onMaxChange={(val) => handleThresholdChange('ph', 'max', val)} />
              <ThresholdRangeInput label="Water Flow Rate" unit="L/min" icon={Waves} minValue={thresholds.waterFlow.min} maxValue={thresholds.waterFlow.max} minLimit={1} maxLimit={20} onMinChange={(val) => handleThresholdChange('waterFlow', 'min', val)} onMaxChange={(val) => handleThresholdChange('waterFlow', 'max', val)} />
              <ThresholdRangeInput label="Air Humidity" unit="%" icon={Droplets} minValue={thresholds.airHumidity.min} maxValue={thresholds.airHumidity.max} minLimit={30} maxLimit={90} onMinChange={(val) => handleThresholdChange('airHumidity', 'min', val)} onMaxChange={(val) => handleThresholdChange('airHumidity', 'max', val)} />
              <ThresholdRangeInput label="Grow Light Intensity" unit="Lux" icon={Sun} minValue={thresholds.lightIntensity.min} maxValue={thresholds.lightIntensity.max} minLimit={100} maxLimit={3000} onMinChange={(val) => handleThresholdChange('lightIntensity', 'min', val)} onMaxChange={(val) => handleThresholdChange('lightIntensity', 'max', val)} />
              <ThresholdRangeInput label="Air Pressure" unit="hPa" icon={Activity} minValue={thresholds.airPressure.min} maxValue={thresholds.airPressure.max} minLimit={980} maxLimit={1040} onMinChange={(val) => handleThresholdChange('airPressure', 'min', val)} onMaxChange={(val) => handleThresholdChange('airPressure', 'max', val)} />
            </div>
          </div>
          ...
          <button
            ...
            disabled={!hasChanges}
            className={`w-full py-4 font-bold rounded-xl shadow-lg transition-all ${hasChanges ? 'bg-emerald-600 hover:bg-emerald-700 text-white hover:shadow-xl' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
          >
